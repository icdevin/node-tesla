...
class Vehicle {
  ...
  summon () {
    ...
    const ws = new WebSocket(
      `${config.SOCKET_BASE_URL}/connect/${this.vehicle.vehicle_id}`,
      null,
      {
        headers: {
        'Authorization': `Basic ${encodedAuthStr}`
    ...
    ws.on('message', (data, flags) => console.log('Message', JSON.parse(data), flags));
    ws.on('error', err => console.error(err));
    ws.on('close', (code, message) => console.log('Close', code, message));
  }
